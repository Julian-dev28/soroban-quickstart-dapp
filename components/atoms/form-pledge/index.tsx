...
import * as token from 'soroban-token-a'
...
function MintButton({ account, symbol, onComplete, decimals }: { decimals: number, account: string; symbol: string, onComplete: () => void }) {
  ...
  return (
    <Button
      ...
      onClick={async () => {
        setSubmitting(true)
        await token.mint({ to: account, amount }, { signAndSend: true })
        setSubmitting(false)
        onComplete()
      }}
...
const FormPledge: FunctionComponent<IFormPledgeProps> = props => {
  ...
  React.useEffect(() => {
    Promise.all([
      token.balance({ id: props.account }),
      token.decimals(),
      token.symbol(),
    ]).then(fetched => {
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      await deposit({
        to: props.account,
        desired_a: BigInt(amount * 10**decimals),
        min_a: BigInt(0),
        desired_b: BigInt(0),
        min_b: BigInt(0),
      }, {
